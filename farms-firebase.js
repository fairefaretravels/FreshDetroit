...
const fd_farmsRef = firebaseReady ? db.collection("farms") : null;

async function getFarms() {
  if (!firebaseReady) return [];
  const snap = await fd_farmsRef.orderBy("name").get();
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}

async function getFarm(id) {
  if (!firebaseReady) return null;
  const snap = await fd_farmsRef.doc(id).get();
  return snap.exists ? { id: snap.id, ...snap.data() } : null;
}

async function addFarm(farm) {
  if (!firebaseReady) throw new Error("Firebase not configured");
  return fd_farmsRef.add({
    ...farm,
    createdAt: Date.now(),
    updatedAt: Date.now()
  });
...
/* ---------------- ADMIN: seed the three preset farms ----------------
   ...
   Requires an authenticated session whose fd_users/{uid} doc has
...
async function seedPresetFarms() {
  ...
  for (const preset of PRESET_FARMS) {
    try {
      const existing = await fd_farmsRef
        .where("preset", "==", true)
  ...
  if (result.errors.length && result.created.length === 0 && result.skipped.length === 0) {
    console.warn("seedPresetFarms: every write failed — most likely this session's fd_users doc isn't role:'admin', or the rules haven't been published yet.");
  }
  return result;
}
